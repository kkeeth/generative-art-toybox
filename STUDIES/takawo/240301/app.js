let graphices = [];
let movers = [];
let offset;
let palette;
const MAX = 300;

function setup() {
  createCanvas(windowWidth, windowHeight);
  palette = random(colorScheme).colors.concat();

  offset = width / 10;

  let iMax = 10;
  for (let i = 0; i < iMax; i++) {
    let g = createGraphics(width, height);
    g.size = iMax - i;
    graphices.push(g);
  }

  for (let i = 0; i < MAX; i++) {
    let pos = createVector(
      random(-offset, width + offset),
      random(-offset, height + offset),
      random(1000),
    );
    movers.push(new Mover(pos, random(graphices), random(palette)));
  }
}

function draw() {
  blendMode(BLEND);
  background(0, map(sin(frameCount / 100), -1, 1, 255, 200));
  blendMode(ADD);
  for (let g of graphices) {
    g.clear();
  }

  for (let mover of movers) {
    mover.update();
    mover.display(mover.target);
  }

  let i = 0;
  for (let g of graphices) {
    drawingContext.filter =
      "blur(" +
      map(
        sin((i++ / graphices.length) * TWO_PI + frameCount / 100),
        -1,
        1,
        1,
        10,
      ) *
        graphices.length +
      "px)";
    image(g, 0, 0);
  }
}

class Mover {
  constructor(pos, target, color) {
    this.pos = pos.copy();
    this.offset = createVector(width / 2, height / 2);
    this.target = target;
    this.d = random(20) * this.target.size;
    this.speed = random() * 3 * (random() > 0.5 ? -1 : 1);
    this.color = color;
  }
  update() {
    this.pos.add(p5.Vector.fromAngle(this.createAngle()).mult(this.speed));
    this.checkEdge();
  }
  checkEdge() {
    if (this.pos.x < -offset) {
      this.pos.x = width + offset;
    }
    if (this.pos.x > width + offset) {
      this.pos.x = -offset;
    }
    if (this.pos.y < -offset) {
      this.pos.y = height + offset;
    }
    if (this.pos.y > height + offset) {
      this.pos.y = -offset;
    }
  }
  createAngle() {
    let n = noise(
      this.offset.x + this.pos.x / width,
      this.offset.y + this.pos.y / height,
      this.pos.z,
    );
    return n * TWO_PI - PI;
  }
  display(target) {
    target.noStroke();
    target.fill(this.color);
    target.circle(this.pos.x, this.pos.y, this.d);
  }
}

function windowResized() {
  resizeCanvas(windowWidth, windowHeight);
}

let colorScheme = [
  {
    name: "Benedictus",
    colors: ["#F27EA9", "#366CD9", "#5EADF2", "#636E73", "#F2E6D8"],
  },
  {
    name: "Cross",
    colors: ["#D962AF", "#58A6A6", "#8AA66F", "#F29F05", "#F26D6D"],
  },
  {
    name: "Demuth",
    colors: ["#222940", "#D98E04", "#F2A950", "#BF3E21", "#F2F2F2"],
  },
  {
    name: "Hiroshige",
    colors: ["#1B618C", "#55CCD9", "#F2BC57", "#F2DAAC", "#F24949"],
  },
  {
    name: "Hokusai",
    colors: ["#074A59", "#F2C166", "#F28241", "#F26B5E", "#F2F2F2"],
  },
  {
    name: "Hokusai Blue",
    colors: ["#023059", "#459DBF", "#87BF60", "#D9D16A", "#F2F2F2"],
  },
  {
    name: "Java",
    colors: ["#632973", "#02734A", "#F25C05", "#F29188", "#F2E0DF"],
  },
  {
    name: "Kandinsky",
    colors: ["#8D95A6", "#0A7360", "#F28705", "#D98825", "#F2F2F2"],
  },
  {
    name: "Monet",
    colors: ["#4146A6", "#063573", "#5EC8F2", "#8C4E03", "#D98A29"],
  },
  {
    name: "Nizami",
    colors: ["#034AA6", "#72B6F2", "#73BFB1", "#F2A30F", "#F26F63"],
  },
  {
    name: "Renoir",
    colors: ["#303E8C", "#F2AE2E", "#F28705", "#D91414", "#F2F2F2"],
  },
  {
    name: "VanGogh",
    colors: ["#424D8C", "#84A9BF", "#C1D9CE", "#F2B705", "#F25C05"],
  },
  {
    name: "Mono",
    colors: ["#D9D7D8", "#3B5159", "#5D848C", "#7CA2A6", "#262321"],
  },
  {
    name: "RiverSide",
    colors: ["#906FA6", "#025951", "#252625", "#D99191", "#F2F2F2"],
  },
];
