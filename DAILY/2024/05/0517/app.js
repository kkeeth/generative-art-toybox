...
function setup() {
  ...
  for (let i = 0; i < NUM; i++) {
    const initailAngle = random(360)
    items.push({
      radius: i * 30,
      angle: initailAngle,
      color: random(random(colorPalette).colors),
      direction: i % 2 === 0 ? 1 : -1,
      fillFlg: random() > 0.5,
      size: random(3, 12),
    })
...
function draw() {
  ...
  for (let { radius, angle, color, direction, fillFlg, size } of items) {
    let x = radius * cos(angle + frameCount * 2 * direction)
    let y = radius * sin(angle + frameCount * 2 * direction)
    push()
    noFill()
    strokeWeight(3)
    stroke(color)
    ...
    direction === 1
      ? arc(
          ...
          radius * 2,
          radius * 2,
          angle + frameCount * 2 * direction - 180,
          angle + frameCount * 2 * direction,
        )
      : arc(
          0,
          0,
