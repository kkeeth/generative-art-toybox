...
function setup() {
  ...
  for (let i = 0; i < W; i += span) {
    const colors = random(cp).colors
    for (let j = 0; j < 10; j++) {
      drawSegment(i, span, j * 10, colors[j % colors.length])
    }
  }
}

function drawSegment(x, w, offsetY, color) {
  stroke(color)
  for (let h = 0; h < W + 100; h += 100) {
    push()
    translate(0, offsetY)
    if ((h / 100) % 2 === 0) {
      line(x, h, x + w, h + 100)
